...
textInfoDiv.setAttribute('class', 'info-container');
gameDiv.appendChild(textInfoDiv);

const resultDiv = document.createElement('h2');
resultDiv.textContent = 'Make Choice';
textInfoDiv.appendChild(resultDiv);

const scoresDiv = document.createElement('div');
scoresDiv.setAttribute('class', 'scores');
textInfoDiv.appendChild(scoresDiv);

const playerScoresDiv = document.createElement('h3');
playerScoresDiv.textContent = 'Your score: 0';
scoresDiv.appendChild(playerScoresDiv);

const computerScoresDiv = document.createElement('h3');
computerScoresDiv.textContent = 'Computer score: 0';
scoresDiv.appendChild(computerScoresDiv);


const choicesDisplayDiv = document.createElement('div');
choicesDisplayDiv.setAttribute('class', 'choices-display');
gameDiv.appendChild(choicesDisplayDiv);

const userChoiceDiv = document.createElement('div');
userChoiceDiv.innerHTML = rockSVG;
choicesDisplayDiv.appendChild(userChoiceDiv);

const computerChoiceDiv = document.createElement('div');
computerChoiceDiv.innerHTML = rockSVG;
...
function playRound(playerSelection, computerSelection) {
    ...
    if (playerSelection === computerSelection){ 
        ...
        userChoiceDiv.setAttribute('class', 'standoff');
        computerChoiceDiv.setAttribute('class', 'standoff');
        return 0;
    }
    if ((playerSelection === 'Paper' && computerSelection === 'Scissors') ||
       ...
       (playerSelection === 'Scissors' && computerSelection === 'Rock')) {
        ...
        userChoiceDiv.setAttribute('class', 'loser');
        computerChoiceDiv.setAttribute('class', 'winner');
        return -1;
    }
    resultDiv.textContent = "You Win! " + playerSelection + " beats " + computerSelection;
    userChoiceDiv.setAttribute('class', 'winner');
    computerChoiceDiv.setAttribute('class', 'loser');
    return 1;
}
...
function startNewGame() {     
    startBtn.addEventListener('click', function(){
        ...
        userChoiceDiv.innerHTML = rockSVG;
        computerChoiceDiv.innerHTML = rockSVG;
        userChoiceDiv.setAttribute('class', '');
        computerChoiceDiv.setAttribute('class', '');
    });
